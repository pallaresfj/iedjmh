<x-filament-widgets::widget class="agro-widget">
    <section class="agro-surface">
        <header class="agro-surface__header">
            <h3 class="agro-surface__title">Solicitudes de Matricula</h3>
        </header>
        ...
        <div class="agro-pqrs-list">
            @forelse ($items as $item)
                <article class="agro-pqrs-item {{ $item['stripe_class'] }}">
                    <div>
                        <p class="agro-pqrs-item__subject">{{ $item['student'] }} • {{ $item['grade'] }}</p>
                        <p class="agro-pqrs-item__meta">
                            {{ $item['submitted_at'] }} • {{ $item['campus'] }} • Doc. {{ $item['document'] }}
                        </p>
                    </div>
            ...
            @empty
                <p class="agro-empty-copy">No hay solicitudes de matricula registradas aun.</p>
            @endforelse
        </div>
    </section>
